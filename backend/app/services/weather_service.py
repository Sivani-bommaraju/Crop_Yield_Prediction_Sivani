...
def analyze_weather(state: str):
    ...
    state_data = weather_df[
        weather_df["state"].str.lower() == state.strip().lower()
    ]
    ...
    if state_data.empty:
        ...
        return {
            ...
            "rainfall_status": "Data Unavailable",
            "temperature_status": "Data Unavailable",
            "impact": "Weather data not available for this state."
        }
    ...
    avg_rainfall = float(
        round(
            state_data["total_rainfall_mm"].mean(),
            2
        )
    )
    ...
    avg_humidity = float(
        round(
            state_data["avg_humidity_percent"].mean(),
            2
        )
    )
    ...
    if rainfall_status == "Low Rainfall":
        ...
        impact = (
            "Heavy rainfall may increase waterlogging "
            "and disease risk."
        )
